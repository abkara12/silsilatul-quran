    ...
    try {
      ...
      await setDoc(
        doc(db, "users", studentUid, "logs", dateKey),
        {
          ...
          sabakDhorMistakes,
          dhorMistakes,
          updatedBy: me?.uid ?? null, // ✅ track who saved
        ...
        <div className="rounded-3xl border border-gray-200 bg-white/70 backdrop-blur p-8 shadow-sm">
          <div className="flex items-start justify-between gap-6">
            <div>
              ...
              <h1 className="mt-1 text-3xl font-semibold tracking-tight">Log work for {studentEmail || "student"}</h1>
              <p className="mt-2 text-gray-700">Submitting for <span className="font-semibold">{dateKey}</span></p>
            </div>

            <div className="flex gap-3">
              <Link className="h-10 px-4 rounded-full border border-gray-200 bg-white/70 hover:bg-white grid place-items-center text-sm" href="/admin">
                Back
              </Link>
              <button
